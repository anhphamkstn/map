...
function initMap() {
    ...
    var onchangeaddress=function () {
        //geocodeAddress(geocoder,marker);
        document.getElementById('distance').value=getDistancebyaddress(geocoder,document.getElementById('start').value,document.getElementById('end').value);
        // alert(getDistancebyaddress(geocoder,"Giải Phóng, Đống Đa, Hà Nội, Vietnam","1 Lê Duẩn, Hanoi, Vietnam"));
    };
    ...
    alert(getDistancebymarker(markers[0],markers[1]));
    //writeToFile();
    //calculateGetDistance();
    //Distanceall();
    //document.getElementById('distance').value=getDistance(p1,p2);
    //getDistancebyaddress(geocoder,document.getElementById('start').value,document.getElementById('end').value);
    //getDistancebymarker(markers[13],markers[17]);
}
var markers = [
...
var p1 = {lat: 21.9970736, lng: 105.8375195};
var p2 = {lat: 21.0020267,lng: 105.8466438};
var d;
...
function calculateGetDistance(){
    for (var i = 0; i < (markers.length-20); i++)
        for (var j = 0; j< (markers.length-21); j++)
        {
...
function writeToFile(){
    alert("beginning");
    var txtFile = "C://xampp/htdocs/JonnyNguyen/resources/views/output.txt";
    alert("end0");
    ...
    alert("end");
    file.close();
    // var fh = fso.open("C:\\xampp\htdocs\JonnyNguyen\resources\views\output.txt");
}
function Write(input1, input2)
{
   
    var fso =  new ActiveXObject("Scripting.FileSystemObject");
    var s = fso.OpenTextFile("C:\\xampp\htdocs\JonnyNguyen\input.txt", true);
